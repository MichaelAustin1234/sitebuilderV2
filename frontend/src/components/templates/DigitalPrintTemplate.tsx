...
interface DigitalPrintTemplateProps {
  toko: Toko & {
    kategoris: Kategori[];
    produks: Produk[];
  };
...
export const DigitalPrintTemplate: React.FC<DigitalPrintTemplateProps> = ({ toko }) => {
  ...
  const [cart, setCart] = useState<CartDigitalItem[]>([]);

  const layout = toko.konfigurasi_layout || {};
  const token = toko.template?.token_desain || {};
  const accentColor = layout.warna_aksen || token.warna_aksen || '#06B6D4';

  const heroTitle = layout.teks_kustom?.hero_title || 'Aset Digital & Percetakan Stiker Custom High-Resolution 300 DPI';
  const aboutUs = layout.teks_kustom?.about_us || 'Studio kreatif penyedia template desain Canva/Figma, e-book panduan bisnis, dan cetak stiker vinyl waterproof custom.';
  const whatsapp = layout.kontak?.whatsapp || '083811223344';
  const alamat = layout.kontak?.alamat || 'BSD City, Tangerang Selatan';
  ...
  const handleSelectFormat = (produkId: number, fmt: string) => {
    setSelectedFormats((prev) => ({ ...prev, [produkId]: fmt }));
  };

  const addToCart = (produk: Produk) => {
    const fmt = selectedFormats[produk.id] || '.PNG (High Res)';
    const paper = 'Vinyl Waterproof A3+';

    setCart((prevCart) => {
      const existing = prevCart.find(
        (item) => item.produk.id === produk.id && item.fileFormat === fmt && item.paperSpec === paper
      );
      if (existing) {
        return prevCart.map((item) =>
          item.produk.id === produk.id && item.fileFormat === fmt && item.paperSpec === paper
            ? { ...item, quantity: item.quantity + 1 }
            : item
        );
      }
      return [...prevCart, { produk, fileFormat: fmt, paperSpec: paper, quantity: 1 }];
    });
  };

  const updateQuantity = (produkId: number, fmt: string, paper: string, delta: number) => {
    setCart((prevCart) =>
      prevCart
        .map((item) => {
          if (item.produk.id === produkId && item.fileFormat === fmt && item.paperSpec === paper) {
            const newQty = item.quantity + delta;
            return newQty > 0 ? { ...item, quantity: newQty } : null;
          }
          return item;
        })
        .filter(Boolean) as CartDigitalItem[]
    );
  };
  ...
  const handleSendWhatsAppOrder = () => {
    ...
    let message = `Halo ${toko.nama_toko}, saya ingin membeli produk digital / cetak berikut (sesuai harga di website Anda):\n\n`;
    cart.forEach((item, index) => {
      const subtotal = item.produk.harga * item.quantity;
      message += `${index + 1}. *${item.produk.nama}* [Format: ${item.fileFormat}] x ${item.quantity} @ Rp ${item.produk.harga.toLocaleString('id-ID')} = Rp ${subtotal.toLocaleString('id-ID')}\n`;
    });

    message += `\n*TOTAL HARGA PESANAN*: Rp ${totalPrice.toLocaleString('id-ID')}\n\nMohon info untuk proses pengiriman file / cetak. Terima kasih!`;
  ...
  const handleDirectWhatsAppChat = () => {
    const cleanPhone = whatsapp.replace(/\D/g, '');
    const formattedPhone = cleanPhone.startsWith('0') ? '62' + cleanPhone.slice(1) : cleanPhone;
    const text = encodeURIComponent(`Halo ${toko.nama_toko}, saya pengunjung toko Anda di website. Saya tertarik dengan produk digital Anda dan ingin bertanya.`);
    window.open(`https://wa.me/${formattedPhone}?text=${text}`, '_blank');
  };

  return (
    <div className="min-h-screen bg-[#090D16] text-[#F8FAFC] font-sans pb-32 relative">
      
      {/* Top Header */}
      <header className="bg-[#0D1322] border-b-2 border-slate-800 py-4 px-6 sticky top-0 z-30 shadow-2xl font-mono">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl md:text-2xl font-black tracking-tight text-white">{toko.nama_toko}</h1>
      ...
      {/* --- HERO CYBER STUDIO --- */}
      <section className="bg-gradient-to-b from-[#0D1322] via-[#090D16] to-[#090D16] py-12 px-6 border-b border-slate-800">
        <div className="max-w-5xl mx-auto text-center">
          {bannerUrl && (
            <div className="mb-8 rounded-2xl overflow-hidden shadow-2xl border-2 border-slate-800">
          ...
          <span className="inline-block text-xs font-mono font-bold uppercase tracking-widest px-3 py-1 bg-slate-900 border rounded mb-3 shadow-xs" style={{ color: accentColor, borderColor: `${accentColor}40` }}>
            ⚡ CYBER DIGITAL ASSETS & PRINT CREATIVE
          </span>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
          ...
          <button
            ...
            style={
              selectedKategoriId === null
                ...
                : { backgroundColor: '#161F33', color: '#F8FAFC' }
            }
          >
            Semua Aset ({toko.produks.length})
          </button>
        ...
        {filteredProduks.length === 0 ? (
          <div className="bg-[#161F33] p-12 text-center text-xs font-mono text-slate-300 font-bold rounded-xl border border-slate-800">
            Belum ada produk digital dalam kategori ini.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProduks.map((produk) => {
              const currentFmt = selectedFormats[produk.id] || '.PNG (300 DPI)';
              const inCart = cart.find(
                (item) => item.produk.id === produk.id && item.fileFormat === currentFmt
              );

              return (
                <div
                  key={produk.id}
                  className="bg-[#0D1322] border-2 rounded-2xl p-5 flex flex-col justify-between hover:border-cyan-500 transition"
                ...
                >
                  <div>
                    ...
                    {/* Spec Selectors */}
                    <div className="pt-3 border-t border-slate-800 font-mono space-y-2">
                      <div>
                        <span className="text-[11px] font-bold uppercase text-slate-300 block mb-1">FORMAT FILE:</span>
                        <div className="flex gap-1.5">
                          {['.PNG', '.PDF', '.SVG'].map((fmt) => (
                            <button
                              key={fmt}
                              onClick={() => handleSelectFormat(produk.id, fmt)}
                              className="px-2.5 py-1 text-[11px] font-bold rounded transition border"
                              style={
                                currentFmt.includes(fmt)
                                  ? { backgroundColor: accentColor, color: '#090D16', borderColor: accentColor }
                                  : { backgroundColor: '#161F33', color: '#F8FAFC', borderColor: '#1E293B' }
                              }
                            >
                              {fmt}
                            </button>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-800 mt-4 font-mono">
                    <div>
                      <span className="text-[10px] font-bold uppercase text-slate-300 block">HARGA ASET</span>
                    ...
                    {inCart ? (
                      <div className="flex items-center gap-2 bg-slate-800 text-white px-3 py-1.5 rounded-lg border border-slate-700">
                        <button onClick={() => updateQuantity(produk.id, currentFmt, 'Vinyl Waterproof A3+', -1)} className="font-bold hover:opacity-80">-</button>
                      ...
                      <button
                        ...
                        className="px-4 py-2 text-slate-950 text-xs font-bold uppercase rounded-lg shadow transition"
      ...
      {/* Floating Cart Bar */}
      {totalItems > 0 && (
        <div className="fixed bottom-0 inset-x-0 bg-[#0D1322] text-[#F8FAFC] border-t-2 shadow-2xl z-40 p-4 font-mono" style={{ borderColor: accentColor }}>
          <div className="max-w-6xl mx-auto flex justify-between items-center gap-4">
            <div className="flex items-center gap-3">
              <span className="h-8 w-8 text-slate-950 font-bold rounded-full flex items-center justify-center text-xs" style={{ backgroundColor: accentColor }}>
                {totalItems}
              </span>
            ...
            <button
              ...
              className="px-5 py-2 text-slate-950 text-xs font-bold uppercase rounded shadow"
              style={{ backgroundColor: accentColor }}
            >
              💬 KIRIM ASET VIA WA
            </button>
      ...
      {selectedProdukDetail && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 font-mono">
          <div className="bg-[#0D1322] rounded-2xl max-w-md w-full p-6 border-2" style={{ borderColor: accentColor }}>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-800">
              ...
              <button
                ...
                className="px-4 py-2 text-slate-950 text-xs font-bold rounded"
                style={{ backgroundColor: accentColor }}
              >
                + Tambah Aset
              </button>
